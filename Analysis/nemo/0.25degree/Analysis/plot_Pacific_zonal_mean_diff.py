...
data2 = xr.open_mfdataset(list2, chunks=xr_chunks2)
...
# Pacific zonal mean bias
data1 = data1*maskvarP
data2 = data2*maskvarP
lat = np.copy(grid.nav_lat*maskvarP)
#lat[lat==0] = np.nan
...
titlename = expid2+'_Pacific_diff_years_'+np.str(fyear)+'_'+np.str(lyear)
plt.title(titlename)
plt.tight_layout()
