...
# SST bias
data = data1*maskvarG
woadata = woadata1*maskvarG
sstmodel = np.copy(data.thetao[:,:,600,600].mean(axis=0))
sstwoaann = np.copy(woadata.SST[:,:,600,600].mean(axis=0))
#sstmodel = np.copy(data.thetao[:,0,:,:].mean(axis=0))
#sstwoaann = np.copy(woadata.SST[:,0,:,:].mean(axis=0))
sys.exit()


[lon,lat,sstdiff] = enable_global(data1.nav_lon,data1.nav_lat,sstmodel-sstwoaann)
...
im1 = m.pcolormesh(np.transpose(lon),np.transpose(lat),np.transpose(np.ma.masked_invalid(sstdiff))
                  ,shading='flat',cmap='RdBu_r',vmin=-5,vmax=5,latlon=True)      
cb = m.colorbar(im1,"right", size="5%", pad="15%") #,ticks=[-4, -3, -2, -1, 0, 1, 2, 3, 4]) # pad is the distance be 

titlename = expid1+'_SST_bias_years_'+np.str(fyear)+'_'+np.str(lyear)
plt.title(titlename)
plt.tight_layout()
