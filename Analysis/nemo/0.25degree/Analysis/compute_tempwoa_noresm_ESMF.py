...
# one degree
# 0.25 degree
grid_file = '/cluster/projects/nn2345k/milicak/mesh_mask_eORCA025.nc'
grid1 = 'orca0_25degree_gridinfov2.nc'
datagrid = xr.open_dataset(grid_file)

grid2 = '/tos-project1/NS2345K/noresm_diagnostics/packages/MICOM_DIAG/obs_data/WOA13/0.25deg/woa13_decav_t00_04.nc'
...
nx = woadata.shape[0]
ny = woadata.shape[1]
dnm = np.zeros([nx,ny,75])
for ii in xrange(0,nx):
    for jj in xrange(0,ny):
        dnm[ii,jj,:] = np.interp(datagrid.gdept_1d,tempsrc.depth,woadata[ii,jj,:])
...
df.to_netcdf('tempwoa_orca0_25deg.nc')
