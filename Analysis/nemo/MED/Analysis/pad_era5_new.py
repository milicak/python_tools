...
datadir = '/okyanus/users/milicak/dataset/ERA5/MED/subset/'
outdir = '/okyanus/users/milicak/dataset/ERA5/MED/padded/'
...
yr = 1993
# for year in range(yr,2023):
for year in range(yr,1995):
    print(year)
    for f, f1 in era5_dict.items():
        print(f)
        # open the file for current year
        out = xr.open_dataset(f"{datadir}/{f}_{year}.nc")
        ...
        if f1=='huss':
            ...
            out[f1].attrs['long_name'] = '2 meter Dew Temperature'
            out[f1].attrs['units'] = 'K'
            out['height'] = 2.0
