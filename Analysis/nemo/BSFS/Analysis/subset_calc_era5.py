...
xinds = np.arange(92,177)
yinds = np.arange(164,213)
...
subdir = '/okyanus/users/milicak/dataset/ERA5/BlackSea_nemo/subset/'
...
for f in era5_dict.keys():
    ...
    for y in years:
        ...
        if f=='ERA5_snowfall':
            ...
            ds['sf'] = ds.sf*1e3/3600
            ds['sf'].attrs = {'units': 'kg m-2 s-1','long_name': 'Snow fall Rate'}
            ds.to_netcdf(str(subdir + f + '_' + str(y) + ".nc"),format="NETCDF4_CLASSIC")
            ds.close()

        if 'total_rain_rate' not in f and 'specific_humidity' not in f and 'snowfall' not in f:
