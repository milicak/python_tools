...
for tind in range(731,984): 
    print(tind)
    inputfile = root_folder + 'REG_' + str(tind) + '_' + str(tind+1) + '_0_' + variable + '_0.01.nc'
    ds = xr.open_dataset(inputfile)
    # extrapolate  ocean values into the land                         
    drowned_var = flood_kara(ds[variable], xdim='lon', ydim='lat', zdim='level')  
    ...
    ds1 = ds1.ffill(dim='z')                           
    outputfile1 = root_folder + '/uTSS_OBC_daily_2021_' + variable + '_shyfem_levels_sea_over_land_low_res_' +  str(tind).zfill(3) + '.nc' 
