...
for prename in prenames:
    ...
    bias_salt = df.Salt_bias/len(stationames)
    bias_temp = df.Temp_bias/len(stationames)
    df3 = pd.DataFrame({"Temp_bias": np.copy(bias_temp[:len(df1.zr_obs)]),
                        "Salt_bias": np.copy(bias_salt[:len(df1.zr_obs)]),
                       "zr_obs": np.copy(df1['zr_obs'])})

    fname = 'obs_ctd_netcdf/' + prename + 'bias.csv'
