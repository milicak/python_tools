...
def boxCut(file_in, base, box):
    ...
    with fiona.open(base + "output.shp", 'w', **meta) as output:
        for i in xrange(int(len(data) / 1000) + 1):
    ...
    def toGeo(self, data):
        ...
        lines = ["Line(IL + %d) = {IP + %d : IP + %d, IP + %d};" % (
            i, sizes[i], sizes[i + 1] - 1, sizes[i])
                 for i in xrange(len(sizes) - 1)] + ["//****lastClosed=IL+%d" % (len(sizes) - 1)]
...
def main():
    ...
    # path to shapefile
    mn = fiona.open('/users/home/mi19918/coastlines-WGS84/lines.shp')
    # mn = fiona.open('/users/home/mi19918/coastlines-split-4326/lines.shp')
    ...
    # box = Box( 22.3, 39, 44, 48)  # red sea
    box = Box( 30, 36, 32, 37)  # red sea
...
outName = 'Antalya_simplify0_0001'
# resol = 0.1
