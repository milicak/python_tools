...
def setCONV_OUS(bas,day,simul):
    ...
    # f.write('#SBATCH --dependency=$(squeue --noheader --format --name nos2nc%s)\n\n' % (day))
    # f.write('#SBATCH --dependency=$(squeue --noheader --format %i --name nos2nc%s)\n\n' % (day))
    ...
    f.write('/okyanus/users/milicak/models/shyfem-share/fembin/memory -s $sim_ous_name\n')
    ...
    f.write('time /okyanus/users/milicak/models/shyfem_serial/fem3d/ous2nc < "$apn2nc" > ous2nc.log 2>&1\n\n')
    f.write('sleep 10\n\n')
...
def setCONV_NOS(bas,day,simul):
    ...
    f.write('sleep 10\n\n')
    f.write('cdo -f nc4 -z zip_4 copy out_nos.nc out2_nos.nc\n')
    f.write('sleep 10\n\n')
    f.write('rm -f out_nos.nc \n')
    f.write('sleep 10\n\n')
    f.write('cdo timavg out2_nos.nc out3_nos.nc \n')
    f.write('sleep 10\n\n')
    f.write('rm -f out2_nos.nc \n')
    f.write('sleep 10\n\n')
    f.write('mv out3_nos.nc ../%s/%s%s%s%s.nos.nc\n'% (out_folder,simul,str(day).zfill(4),regroot,boxname))
    f.write('echo \"Job completed at: \" `date`\n\n')
    f.close()
...
def setCONV_WND(bas,day,simul):
    ...
    f.write('source /okyanus/users/milicak/loadmodule_shyfem\n')
    # f.write('cd out_%s\n\n'% (str(day).zfill(4)))
    f.write('/okyanus/users/milicak/models/shyfem-share/fembin/memory -s $sim_wnd_name\n')
    f.write('/okyanus/users/milicak/models/shyfem-share/fembin/memory -b $sim_bas_name\n\n')
    f.write('apn2nc=\"apnnos2nc.str\"\n')
    f.write('rm $apn2nc\n\n')
    f.write('echo -e " " >> "$apn2nc"\n')
    ...
    f.write('ncwa -d level,0 -a level out_wnd.nc out2_wnd.nc\n')
    f.write('sleep 10\n\n')
    f.write('mv out2_wnd.nc out_wnd.nc \n')
    f.write('sleep 10\n\n')
    f.write('cdo -f nc4 -z zip_4 copy out_wnd.nc out2_wnd.nc\n')
    f.write('sleep 10\n\n')
    f.write('rm -f out_wnd.nc \n')
    f.write('sleep 10\n\n')
    f.write('cdo timavg out2_wnd.nc out3_wnd.nc \n')
    f.write('sleep 10\n\n')
    f.write('rm -f out2_wnd.nc \n')
    f.write('sleep 10\n\n')
    f.write('mv out3_wnd.nc ../%s/%s%s%s%s.wnd.nc\n'% (out_folder,simul,str(day).zfill(4),regroot,boxname))
    f.write('echo \"Job completed at: \" `date`\n\n')
    f.close()
