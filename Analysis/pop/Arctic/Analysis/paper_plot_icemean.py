...
for i in xrange(0,len(projects)):
    ...
    #im1 = m.pcolormesh(lon,lat,np.transpose(np.ma.masked_invalid(ICEareaMarch)),
    #shading='flat',cmap=plt.cm.gist_ncar_r,latlon=True)
    #im1 = m.pcolormesh(lon,lat,np.transpose(np.ma.masked_invalid(ICEareaMarch)),
    #shading='flat',cmap=plt.cm.spectral,latlon=True)
    #im1 = m.pcolormesh(lon,lat,np.transpose(np.ma.masked_invalid(ICEareaMarch)),
    #shading='flat',cmap=plt.cm.Blues,
    #im1 = m.pcolormesh(lon,lat,np.transpose(np.ma.masked_invalid(ICEareaMarch)),
    #shading='flat',cmap=discrete_cmap(9, 'gist_ncar_r')
    im1 = m.pcolormesh(lon,lat,np.transpose(np.ma.masked_invalid(ICEareaMarch))
                       ,shading='flat',cmap=cmap_needjet2
                      ,vmin=20,vmax=100,latlon=True)
    # vmax=100, vmin=20,latlon=True)
    #cmap = plt.get_cmap('Blues', 10)
    cb = m.colorbar(im1,"right", size="5%", pad="2%",ticks=[20, 30, 40, 50, 60, 
                                                            70, 80, 90, 100])
    cb.set_label('%')
    #plt.clim(20,110)
    plt.show()
    plt.savefig('paperfigs/'+projects[i]+'_icefrac_March_'+str(av1)+'_'
                +str(av2)+'_years.eps',
    ...
    if(i!=0 and i<4):
        ...
        cb.set_label('%')
        plt.show()
        plt.savefig('paperfigs/'+projects[i]+'_icefrac_diff_March_'+
