...
fig = plt.figure()
for i in xrange(0,4):
    filename=root_folder+projects[i]+'_moc_'+fyear+'_'+lyear+'.mat'
    mat = scipy.io.loadmat(filename)
    amoc26=np.transpose(np.array(mat['AMOC26']))    
    amoc26_fil=my_nanfilterbox(amoc26[:,0],dn)
    #plt_arrays(time,runningMeanFast(amoc26[:,0],10), title="",label=legendnames[i],color=plotcolors[i])
    plt_arrays(time,amoc26_fil, title="",label=legendnames[i],color=plotcolors[i])
plt.xlabel('Time [years]')    
plt.ylabel('AMOC [Sv]')  
plt.ylim(17,24) 
plt.show()
plt.savefig('paperfigs/cold_exps_amocs.eps', format='eps', dpi=1000)    
plt.close(fig)
...
plt.ylim(17,24)   
plt.show()
plt.savefig('paperfigs/warm_exps_amocs.eps', format='eps', dpi=1000)    
plt.close(fig)
...
plt.ylim(17,24)   
plt.show()
plt.savefig('paperfigs/cold_warm_ctrls_amocs.eps', format='eps', dpi=1000)    
plt.close(fig)
