...
for i in ctrls:
    ...
    im1 = m.pcolormesh(lon,lat,np.transpose(np.ma.masked_invalid(Tref_winter)),shading='flat',cmap=cmap_needjet2,latlon=True)
    ...
    #plt.clim(20,110)
    plt.show()
    plt.savefig('paperfigs/'+projects[i]+'_airtemp_winter_'+str(av1)+'_'+str(av2)+'_years.eps', format='eps', dpi=300)    
    plt.clf()
    plt.close(fig)
...
for i in xrange(0,7):
     if(i!=0 and i<4):                  
         ...
         cb.set_label('%')
         plt.show()
         plt.savefig('paperfigs/'+projects[i]+'_airtemp_winter_diff_'+str(av1)+'_'+str(av2)+'_years.eps', format='eps', dpi=300)    
         plt.clf()
         plt.close(fig)
     elif(i>=5):
        fig = plt.figure()
