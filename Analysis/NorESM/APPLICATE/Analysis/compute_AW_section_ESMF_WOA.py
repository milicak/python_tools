...
domask=True
# create locstream
#locstream = ESMF.LocStream(lon_s4.shape[0], name="Atlantic Inflow Section", coord_sys=coord_sys)
...
#locstream["ESMF:Lon"] = lon_s4
#locstream["ESMF:Lat"] = lat_s4
if domask:
    locstream["ESMF:Mask"] = np.array(np.ones(lon_s4new.shape[0]), dtype=np.int32)
    #locstream["ESMF:Mask"] = np.array(np.ones(lon_s4.shape[0]), dtype=np.int32)
...
#m.plot(xpt[20],ypt[20],'-o',color='g')
#m.plot(xpt[21],ypt[21],'-o',color='g')
...
#grid2 = "/mnt/fimmhome/Analysis/NorESM/climatology/Analysis/WOA09_temp.nc"
#grid2 = "/mnt/fimm/RUNS/woa13_decav_t_annual_01.nc"
datafname = grid2
...
#secfield = np.zeros((lon_s4.shape[0],tempwoa.shape[0]))
#sys.exit()
