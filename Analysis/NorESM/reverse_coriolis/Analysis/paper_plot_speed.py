...
for i in xrange(0,1):
    ...
    uvel=np.array(mat['uvellvl'])
    vvel=np.array(mat['vvellvl'])
    u1=np.copy(np.squeeze(uvel[:,:-1,0]))
    v1=np.copy(np.squeeze(uvel[:,:-1,0]))
    [lonu,latu,u1]=enable_global(lon,lat,np.transpose(u1))
    [lon,lat,v1]=enable_global(lon,lat,np.transpose(v1))
    fig = plt.figure()
    ...
    im1 = m.pcolormesh(np.transpose(lon-110),np.transpose(lat),np.transpose(np.ma.masked_invalid(sst))
                      ,shading='flat',cmap=palette,vmin=-2,vmax=32,latlon=True)
...
#plt.show()
plt.savefig('paperfigs/'+projects[i]+'_sst.eps', bbox_inches='tight',format='eps', dpi=200)
plt.clf()
plt.close(fig)
