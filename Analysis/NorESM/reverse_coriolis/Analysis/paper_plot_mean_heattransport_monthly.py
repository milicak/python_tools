...
lhflx=np.zeros([ny,nx])
    
for year in xrange(np.int(fyear),np.int(lyear)+1):
    for month in xrange(1,13):
        # control
        #filename = root_folder+projects[0]+'/atm/hist/'+projects[0]+'.cam2.h0.'+str(year).zfill(4)+'-'+str(month).zfill(2)+'.nc'
        # reverse Coriolis
        filename = root_folder+projects[0]+'/atm/hist/'+projects[0]+'.cam.h0.'+str(year).zfill(4)+'-'+str(month).zfill(2)+'.nc'
    
        precc = ncread_time_surface(filename,'PRECC',0,1,nx,ny)*mw[month-1]
        precl = ncread_time_surface(filename,'PRECL',0,1,nx,ny)*mw[month-1]
        lhflx = lhflx + ncread_time_surface(filename,'LHFLX',0,1,nx,ny) \
            *mw[month-1]/(Lv*rhow)
        #total precipitation
...
lhflx = lhflx/(np.float(lyear)-np.float(fyear)+1)
EminusP = lhflx-prect
# convert m/s to mm/day
EminusP = EminusP * (1000.*86400)
...
im1 = m.pcolormesh(lon,lat,np.ma.masked_invalid(EminusP),shading='flat',cmap='jet'
                      ,vmin=-13,vmax=5,latlon=True)
...
plt.savefig('paperfigs/'+projects[0]+'_EminusP_mean.eps', bbox_inches='tight',format='eps', dpi=200)
plt.clf()
plt.close(fig)
