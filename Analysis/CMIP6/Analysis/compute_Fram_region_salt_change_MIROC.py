...
root_dir = '/tos-project1/NS9252K/CMIP6/ssp585/MIROC6/r1i1p1f1/'
fname = root_dir + 'so_Omon_MIROC6_ssp585_r1i1p1f1_gn_208501-209412.nc'

dfs = xr.open_dataset(fname)

root_dir = '/tos-project1/NS9252K/CMIP6/historical/MIROC6/r1i1p1f1/'
fname = root_dir + 'so_Omon_MIROC6_historical_r1i1p1f1_gn_199001-199912.nc'
...
dfc.to_netcdf('MIROC6_salt_control.nc')
dfs.to_netcdf('MIROC6_salt_ssp585.nc')


for i in range(0,dfs.so.shape[2]):
    for j in range(0,dfs.so.shape[1]):
        if mask[j,i] == 1:
            plt.plot(dfs.so[:,j,i]-dfc.so[:,j,i],-dfs.lev,'silver')
