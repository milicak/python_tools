...
# set new dataset
dsnew = dft.to_dataset(name='temp')
dsnew['salt'] = dst
dsnew['ssh'] = dzt
...
# regridder = xe.Regridder(ds, target_t, filename='regrid_shyfem_tracers.nc', **regrid_kws)
regridder = xe.Regridder(ds, target_t, 'nearest_s2d')
dsnew.load()

#apply regridder
dr_out = regridder(dsnew['ssh'])
ssh = np.copy(dr_out[:,0,:,:])

dr_out = regridder(dsnew['temp'])
vartemp = np.copy(dft[:,:,:,:])
nk = vartemp.shape[1]
nj = vartemp.shape[2]
ni = vartemp.shape[3]
