...
for year in range(1996,2018):
    ...
    # build regridder
    # if exists('patch_330x720_1300x2000.nc'):
    if exists('patch_330x720_1300x2000_peri.nc'):
        regridder2 = xe.Regridder(out2, ds2, 'patch',reuse_weights=True, periodic=True)
        # regridder2 = xe.Regridder(out2, ds2, 'patch',reuse_weights=True, periodic=False)
    else:
    ...
    out_file = '/okyanus/users/milicak/dataset/MOM6/Arctic_Copernicus/SODA_sponge_' + str(year) + '.nc'
    dr_out.to_netcdf(
        out_file,
        unlimited_dims=['time'],
