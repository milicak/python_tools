...
datadir = '/okyanus/users/milicak/dataset/ERA5/Arctic_Copernicus/subset/'
...
for year in range(2008,2009):
    ...
    for f, f1 in era5_dict.items():
        ...
        ds_ext = xr.Dataset({
            f1 : xr.DataArray(
                        data   = np.zeros((len(range(0,out.dims['time'])),
                                           len(range(0,out.dims['latitude'])),
                                           len(range(0,out.dims['longitude']+1)))),   # enter data here
                        dims   = ['time', 'latitude', 'longitude'],
                        coords = {'time': times, 'latitude' : latitudes, 'longitude' : longitudes},
                        attrs  = {
                            # 'units'     : out[f1].attrs['units']
                            }
                        )})

        ds_ext[f1].values[:,:,0:len(out['longitude'])] = out[f1].values
        ds_ext[f1].values[:,:,len(out['longitude']):len(out['longitude']) + 1] =  out[f1][:,:,0:1].values
        ds_ext.to_netcdf(f'{outdir}{f}_{year}.nc', format="NETCDF4_CLASSIC", encoding=encodings, unlimited_dims='time')
        out.close()
        ds_ext.close()
