...
im1 = m.pcolormesh(lon,lat,ma.masked_where(gr.D==0,dfs[:,:]),
                   cmap='nice_gfdl',vmin=-1,vmax=18,latlon=True, rasterized=True)
...
cbar.set_label(r'$^\circ$C',rotation=0,y=1.02,labelpad=-45,fontsize=14)
ax1.text(lon1,lat1,'a)',fontsize=14)

m.ax=ax2
m.drawcoastlines()
m.fillcontinents(color='grey');
im2 = m.pcolormesh(lon,lat,dfs-np.copy(df.temp_woa[1,:,:]),
...
plt.savefig('paperfigs/temp_sst_bias.png', bbox_inches='tight',format='png',dpi=300)
