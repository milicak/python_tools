...
if __name__ == '__main__':
    coast = get_coast_mask('/okyanus/users/milicak/dataset/MOM6/NA12/ocean_mask.nc')
    hgrid = xarray.open_dataset( '/okyanus/users/milicak/dataset/MOM6/NA12/ocean_hgrid.nc')
    # For NWA: subset GloFAS to a smaller region containing NWA.
    glofas_subset = dict(lat=slice(81, -29), lon=slice(-104, 57))
    glofas_subset2 = dict(lat=slice(81, -29), lon=slice(-104+360,57+360))
    # GloFAS upstream area from
    # https://confluence.ecmwf.int/download/attachments/143039724/upArea.nc?version=5&modificationDate=1606390153801&api=v2
    # The metadata says it is GloFAS version 2.1, but it looks like a good match for 3.1
    uparea = xarray.open_dataarray('/okyanus/users/milicak/dataset/MOM6/NA12/upArea.nc').sel(**glofas_subset)
    ...
    for y in range(1996, 1997):
        print(y)
        files = [f'/okyanus/users/milicak/dataset/GLOFAS/glofas-era5_{y}.nc' for y in [y-1, y, y+1]]
        glofas = (
            xarray.open_mfdataset(files, combine='by_coords')
            .sel(time=slice(f'{y-1}-12-31 00:00:00', f'{y+1}-01-01 12:00:00'),
                 **glofas_subset2)
            .dis24.chunk({'time': -1})
        )
        out_file = f'/okyanus/users/milicak/dataset/MOM6/NA12/glofas_out_1996.nc'
