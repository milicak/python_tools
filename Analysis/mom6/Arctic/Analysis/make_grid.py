...
# Dimensions
rg.createDimension('ni',sni)
rg.createDimension('nip1',sni+1)
rg.createDimension('nj',snj)
rg.createDimension('njp1',snj+1)
rg.createDimension('string',255)
# Variables
hx = rg.createVariable('x','float32',('njp1','nip1',))
hx.units = 'degrees'
hy = rg.createVariable('y','float32',('njp1','nip1',))
hy.units = 'degrees'
hdx = rg.createVariable('dx','float32',('njp1','ni',))
hdx.units = 'meters'
hdy = rg.createVariable('dy','float32',('nj','nip1',))
hdy.units = 'meters'
harea = rg.createVariable('area','float32',('nj','ni',))
harea.units = 'meters^2'
hangle = rg.createVariable('angle_dx','float32',('njp1','nip1',))
...
depth_gebco = np.reshape(np.copy(gr.z),(np.copy(gr.dimension[0]),np.copy(gr.dimension[1])))
lon_gebco,lat_gebco = np.meshgrid(lon_gebco,lat_gebco)


tree = cKDTree(np.vstack((lon_gebco.flatten(),lat_gebco.flatten())))
